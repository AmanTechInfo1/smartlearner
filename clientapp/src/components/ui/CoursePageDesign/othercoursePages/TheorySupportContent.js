...
/* ========================================================================
   MAIN PAGE
   ======================================================================== */
export default function TheorySupportContent() {
  ...
  useEffect(() => {
    dispatch(getCourseBySlug("theory-support"));
  }, [dispatch]);
  ...
  const manualCategory = data.find((category) => category._id === "theory support");
  ...
  const hourPackages =
    manualCategory?.data?.filter((product) => {
      ...
      return name.includes("hour") || name.includes("hours") || name.includes("online");
    }) || [];
