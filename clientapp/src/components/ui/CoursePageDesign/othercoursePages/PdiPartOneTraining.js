...
/* ========================================================================
   MAIN PAGE
   ======================================================================== */
export default function PdiPartOneTraining() {
  ...
  useEffect(() => {
    dispatch(getCourseBySlug("instructor-training-part-one"));
  }, [dispatch]);
  ...
  const manualCategory = data.find((category) => category._id === "instructor training part one");
  ...
  const hourPackages =
    manualCategory?.data?.filter((product) => {
      ...
      return name.includes("platinum") || name.includes("gold") || name.includes("silver") || name.includes("hours");
    }) || [];
