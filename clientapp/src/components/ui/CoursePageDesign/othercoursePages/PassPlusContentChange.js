...
/* ========================================================================
   MAIN PAGE
   ======================================================================== */
export default function PassPlusContentChange() {
  ...
  useEffect(() => {
    dispatch(getCourseBySlug("pass-plus"));
  }, [dispatch]);
  ...
  const manualCategory = data.find((category) => category._id === "pass plus");
  ...
  const hourPackages =
    manualCategory?.data?.filter((product) => {
      ...
      return name.includes("course") || name.includes("hour");
    }) || [];
