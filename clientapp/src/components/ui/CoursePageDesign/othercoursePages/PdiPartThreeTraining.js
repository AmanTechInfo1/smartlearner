...
/* ========================================================================
   MAIN PAGE
   ======================================================================== */
export default function PdiPartThreeTraining() {
  ...
  useEffect(() => {
    dispatch(getCourseBySlug("instructor-training-part-three"));
  }, [dispatch]);
  ...
  const manualCategory = data.find(
    (category) => category._id === "instructor training part three",
  );
  ...
  const hourPackages =
    manualCategory?.data?.filter((product) => {
      ...
      return name.includes("complete course") || name.includes("online");
    }) || [];
  ...
  const getProductRoute = (productName) => {
    ...
    return PART_THREE_ROUTES[normalizedName];
  };
