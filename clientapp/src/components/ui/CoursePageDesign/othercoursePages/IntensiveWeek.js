...
/* ========================================================================
   MAIN PAGE
   ======================================================================== */
export default function IntensiveWeek() {
  ...
  useEffect(() => {
    dispatch(getCourseBySlug("intensive"));
  }, [dispatch]);
  ...
  const manualCategory = data.find((category) => category._id === "intensive");
  ...
  const hourPackages =
    manualCategory?.data?.filter((product) => {
      ...
      return name.includes("hour") || name.includes("hours");
    }) || [];
