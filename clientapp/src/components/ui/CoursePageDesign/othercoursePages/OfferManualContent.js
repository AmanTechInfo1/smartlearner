...
/* ========================================================================
   MAIN PAGE
   ======================================================================== */
export default function OfferManualContent() {
  ...
  useEffect(() => {
    dispatch(getCourseBySlug("offer-manual"));
  }, [dispatch]);
  ...
  const manualCategory = data.find(
    (category) => category._id === "offers manual",
  );
  ...
  const beginnersPackageIndex =
    manualCategory?.data?.findIndex(
      (product) => product.name?.trim() === "10 Hours Manual",
    ) ?? -1;
